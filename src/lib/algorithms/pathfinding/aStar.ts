...
export const aStar = (
  ...
  while (untraversedTiles.length > 0) {
    ...
    if (currentTile) {
      ...
      const neighbors = getNeighbours(grid, currentTile); // Get untraversed neighbors of the tile
      for (let i = 0; i < neighbors.length; i += 1) {
        ...
        if (distanceToNeighbor < neighbors[i].distance) {
          ...
          removeFromQueue(neighbors[i], untraversedTiles); // Remove the neighbor from the queue
